...
namespace NyAR.MarkerSystem.Utils {

#pragma warning disable 0108

  public class ARMarkerList : List<ARMarkerList.Item> {
    ...
    public bool Update(INyARPerspectiveCopy i_pix_drv, SquareStack.Item i_sq) {
      ...
      for (int i = this.Count - 1; i >= 0; i--) {
        ...
        if (!target.matchpatt.evaluate(diff, this._patt_result)) { continue; }

        //敷居値をチェック
        if (this._patt_result.confidence < this._configense_th) { continue; }
