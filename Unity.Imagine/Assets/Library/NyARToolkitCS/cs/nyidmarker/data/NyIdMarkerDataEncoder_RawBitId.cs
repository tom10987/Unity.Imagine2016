...
namespace NyAR.NyIdMarker {
  ...
  public class NyIdMarkerDataEncoder_RawBitId : NyIdMarkerDataEncoder_RawBit {
    ...
    public override bool Encode(NyIdMarkerPattern i_data, INyIdMarkerData o_dest) {
      //対象か調べるん
      if (i_data.ctrl_domain != 0) { return false; }

      //受け入れられるMaskは0のみ
      if (i_data.ctrl_mask != 0) { return false; }

      //受け入れられるModelは5未満
      if (i_data.model >= 5) { return false; }

      //エンコードしてみる
      if (!base.Encode(i_data, this._tmp)) { return false; }
